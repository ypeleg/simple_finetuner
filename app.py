...
class UI():
    ...
    def base_model_block(self):
        self.model_name = gr.Dropdown(label='Base Model', choices=MODELS)# , value = MODELS[0])
    ...
    def training_launch_block(self):
        with gr.Row():
            with gr.Column():
                self.new_lora_name = gr.Textbox(label='New PEFT Adapter Name', value='lora_hebrew')
            with gr.Column():
    ...
    def layout(self):
        with gr.Blocks() as demo:
            with gr.Row():
                with gr.Column():
                    gr.HTML("""<h2>
                    <a style="text-decoration: none;" href="https://github.com/lxe/simple-llama-finetuner">🦙 Simple LLM Finetuner</a>&nbsp;
    ...
    def run(self):
        self.ui = self.layout()
        self.ui.queue().launch(show_error=True, share=SHARE or True, server_name=SERVER_HOST, server_port=SERVER_PORT)
